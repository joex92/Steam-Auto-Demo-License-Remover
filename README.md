...
To avoid triggering Steam rate limits, the deletion interval defaults to 6 to 8 minutes and is not recommended to shorten.  
...
- Filter by custom words separated by commas.
...
- on SteamDB Free Package script page, Auto ignore all removed games and by custom filter as well.
...
3. After the page loads, check the box for all free titles or demo titles only
4. Type the words you would want to remove as well or to not remove.
5. Click the "🧹Start cleaning" button to start automatic removal.  
6. Keep the page open and wait until the script finishes all operations.
7. (OPTIONAL) Navigate to: https://steamdb.info/freepackages/
8. (OPTIONAL) After the page loads, click the "Ignore all Demo packages"
9. (OPTIONAL) You can start the script to add all free packages except Demo packages.
...
This script is developed by [PeiqiLi](https://github.com/PeiqiLi-Github) for personal use only. Use at your own risk. Neither PeiqiLi nor JoeX92 are responsible for any damage or loss caused. Please operate carefully.

---

## Feedback

You are welcome to submit issues or suggestions [on GitHub](https://github.com/joex92/Steam-Auto-Demo-License-Remover/issues)
