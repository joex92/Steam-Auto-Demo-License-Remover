...
(function() {
    ...
    async function startCleaning(statusDiv) {
        ...
        for (let i = 0; i < total; ) { 
            ...
            statusDiv.append(`🗑️ [${new Date().toISOString()}]${(retries > 0) ? " [Retries: " + retries + ']' : "" } Removing game #${i + 1}：`, scrollToTitle, `\n`);
            ...
            if (i < total) {
                delay = hasError84 ? Math.pow( randomDelay( 390000, 510000 ), 1 / ( 1 + (( retries - 1 ) / 10 ))) : randomDelay( 500, 1500 );
                avgSum += delay;
